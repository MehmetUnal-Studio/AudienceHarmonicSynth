...
Cosmic Microwave 2.2 does not produce sound. It keeps a silent stereo instrument shell
...
- **`AUDIENCE_SYNTH_BUILD_TESTS`** - defaults to `ON` and builds the MIDI mapping,
  ...
  Atomic integration, Time Field, timed MIDI integration, catalog, and state-migration
  tests.
...
New 2.2 sessions also begin with **Ensemble / Host / 1/16**, four attacks per step,
